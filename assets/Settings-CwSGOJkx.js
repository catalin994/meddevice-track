import{m as We,o as C,p as He,q as pe,s as Ve,t as Ke,v as Qe,w as Ze,x as be,n as P,S as he,y as Ge,j as e,z as qe,R as Te,A as ea,B as aa}from"./index-BOTnf5U1.js";import{a as t,o as ge,m as f,v as Ne,F as _,z as Ee,b as ta,T as sa,w as la,C as Y,e as fe,ai as ia,d as k,$ as na,a7 as ra,aj as je,G as ca,ak as oa,B as ve,p as da}from"./vendor-icons-CEpz8T8z.js";import{C as ma}from"./ConfirmDialog-pf8XxV6T.js";import{g as xa,s as ua,S as we}from"./scanQuality-BG2ILRky.js";import"./vendor-recharts-Dj0FOwL8.js";import"./vendor-db-CxvqBt6T.js";const pa="2026-08-04 09:21",wa=({devices:n,onImport:J,auditLog:y=[],currentUser:r=null,onMigrateFiles:j})=>{t.useRef(null);const[W,ba]=t.useState(We()),[H,Se]=t.useState(W.url||""),[V,Ae]=t.useState(W.key||""),[I,Le]=t.useState(!1),[K,Q]=t.useState(!1),[ha,Ta]=t.useState(null),[x,D]=t.useState(null),[Z,G]=t.useState(!1),[Ce,ke]=t.useState(null),[ye,q]=t.useState(null),[R,ee]=t.useState(!1),[d,ae]=t.useState(null),[O,X]=t.useState(null),[F,te]=t.useState(!1),[v,w]=t.useState(!1),[se,le]=t.useState(0),[N,u]=t.useState(null),h=t.useCallback(async()=>{if(!C){X("Cloud neconfigurat");return}te(!0),X(null);const{count:a,error:s}=await He("devices");s?(X(s.message||"eroare necunoscuta"),ae(null)):ae(a),te(!1)},[]);t.useEffect(()=>{h()},[h,n.length]);const[m,ie]=t.useState(null),[U,M]=t.useState(!1),Ie=t.useCallback(async()=>{M(!0),ie(null);const{data:a,error:s}=await pe("devices");if(s||!a){u({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),M(!1);return}const l=new Set(a.map(i=>String(i.id).trim())),c=new Set(n.map(i=>String(i.id).trim())),o=i=>{var b,L;return((b=n.find($=>$.id===i))==null?void 0:b.name)||((L=a.find($=>String($.id).trim()===i))==null?void 0:L.name)||i};ie({localOnly:[...c].filter(i=>!l.has(i)).map(i=>`${o(i)} (${i})`),cloudOnly:[...l].filter(i=>!c.has(i)).map(i=>`${o(i)} (${i})`)}),M(!1)},[n]),De=t.useCallback(async()=>{if(n.length===0)return;w(!0),le(0),u(null);const{data:a,error:s}=await pe("devices");if(s){w(!1),u({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=Ve(n,a||[]);if(l.length===0){w(!1),u({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await h();return}const{error:c,written:o,skippedColumns:i,oversized:b}=await Ke("devices",l,100,L=>le(L));w(!1),c?u({ok:!1,message:`Urcarea s-a oprit dupa ${o} echipamente: ${c.message||c}`}):b.length>0?u({ok:!1,message:`${o} echipamente urcate, dar ${b.length} nu au incaput (documente atasate prea mari): ${b.slice(0,3).join(", ")}${b.length>3?"...":""}`}):i.length>0?u({ok:!0,message:`${o} echipamente au fost urcate. Atentie: campurile ${i.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):u({ok:!0,message:`${o} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await h()},[n,h]),p=t.useMemo(()=>{let a=0,s=0;return n.forEach(l=>(l.files||[]).forEach(c=>{var o;!c.path&&((o=c.url)!=null&&o.startsWith("data:"))&&(a++,s+=Math.round(c.url.length*.75))})),{count:a,mb:s/(1024*1024)}},[n]),[Re,Oe]=t.useState(()=>xa().id),Xe=t.useCallback(a=>{ua(a),Oe(a)},[]),[S,ne]=t.useState(!1),[T,re]=t.useState({done:0,total:0,label:""}),[ce,E]=t.useState(null),Fe=t.useCallback(async()=>{if(j){ne(!0),E(null),re({done:0,total:0,label:""});try{const a=await j((s,l,c)=>re({done:s,total:l,label:c}));a.error?E(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?E("Nu mai exista documente de mutat — totul e deja in Storage."):E(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){E(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{ne(!1)}}},[j]),B=Qe(r,"manageUsers"),[oe,Ue]=t.useState([]),[de,A]=t.useState(""),g=t.useCallback(async()=>{const a=await Ze();Ue(a)},[]);t.useEffect(()=>{B&&g()},[B,g]);const Me=t.useCallback(async(a,s)=>{A("");const{error:l}=await be(a.id,{role:s,approved:!0});l?A(l):g()},[g]),Be=t.useCallback(async a=>{A("");const{error:s}=await be(a.id,{approved:!a.approved});s?A(s):g()},[g]),ze=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),me=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
