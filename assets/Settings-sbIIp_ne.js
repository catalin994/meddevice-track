...
`,ot="2026-08-31 13:03",dt=i=>{if(i===1)return"zi";const p=i%100;return p===0||p>=20?"de zile":"zile"},ft=({devices:i,invoices:p=[],tasks:f=[],referate:m=[],foundationDocs:N=[],comenzi:d=[],onImport:h,auditLog:o=[],currentUser:r=null,onMigrateFiles:g,deletions:$=[],onRestore:k,canDelete:j=!1,onUnesteSectii:Y,canEdit:A=!1})=>{t.useRef(null);const[J,W]=t.useState(va()),[H,c]=t.useState(J.url||""),[x,T]=t.useState(J.key||""),[L,E]=t.useState(!1),[S,V]=t.useState(!1),[mt,ut]=t.useState(null),[I,le]=t.useState(null),[je,Ae]=t.useState(!1),[la,ia]=t.useState(null),[ie,ra]=t.useState(null),[C,na]=t.useState(null),[Se,ve]=t.useState(!1),[O,we]=t.useState(()=>Va());t.useEffect(()=>{Ga().then(we)},[]);const G=t.useMemo(()=>Ka(i,p,m,N,d),[i,p,m,N,d]),re=t.useCallback(async()=>{ve(!0);const[a,s]=await Promise.all([Za(),Qa()]);ra(a),na(s),ve(!1)},[]);t.useEffect(()=>{re()},[re]);const[ca,Le]=t.useState(null),[ne,Ie]=t.useState(!1),[v,Ce]=t.useState(null),[ce,oe]=t.useState(null),[de,Xe]=t.useState(!1),[K,Z]=t.useState(!1),[Oe,De]=t.useState(0),[P,X]=t.useState(null),U=t.useCallback(async()=>{if(!ae){oe("Cloud neconfigurat");return}Xe(!0),oe(null);const{count:a,error:s}=await wa("devices");s?(oe(s.message||"eroare necunoscuta"),Ce(null)):Ce(a),Xe(!1)},[]);t.useEffect(()=>{U()},[U,i.length]);const[w,Re]=t.useState(null),[me,ue]=t.useState(!1),oa=t.useCallback(async()=>{ue(!0),Re(null);const{data:a,error:s}=await Ye("devices");if(s||!a){X({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),ue(!1);return}const l=new Set(a.map(u=>String(u.id).trim())),n=new Set(i.map(u=>String(u.id).trim())),b=u=>{var R,ee;return((R=i.find(be=>be.id===u))==null?void 0:R.name)||((ee=a.find(be=>String(be.id).trim()===u))==null?void 0:ee.name)||u};Re({localOnly:[...n].filter(u=>!l.has(u)).map(u=>`${b(u)} (${u})`),cloudOnly:[...l].filter(u=>!n.has(u)).map(u=>`${b(u)} (${u})`)}),ue(!1)},[i]),da=t.useCallback(async()=>{if(i.length===0)return;Z(!0),De(0),X(null);const{data:a,error:s}=await Ye("devices");if(s){Z(!1),X({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=La(i,a||[]);if(l.length===0){Z(!1),X({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await U();return}const{error:n,written:b,skippedColumns:u,oversized:R}=await Ia("devices",l,100,ee=>De(ee));Z(!1),n?X({ok:!1,message:`Urcarea s-a oprit dupa ${b} echipamente: ${n.message||n}`}):R.length>0?X({ok:!1,message:`${b} echipamente urcate, dar ${R.length} nu au incaput (documente atasate prea mari): ${R.slice(0,3).join(", ")}${R.length>3?"...":""}`}):u.length>0?X({ok:!0,message:`${b} echipamente au fost urcate. Atentie: campurile ${u.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):X({ok:!0,message:`${b} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await U()},[i,U]),D=t.useMemo(()=>{let a=0,s=0;const l=n=>{n!=null&&n.startsWith("data:")&&(a++,s+=Math.round(n.length*.75))};return i.forEach(n=>{(n.files||[]).forEach(b=>{b.path||l(b.url)}),(n.contracts||[]).forEach(b=>{b.filePath||l(b.fileUrl)})}),f.forEach(n=>(n.attachments||[]).forEach(b=>{b.path||l(b.url)})),[...p,...m,...N,...d].forEach(n=>{n.filePath||l(n.fileUrl)}),{count:a,mb:s/(1024*1024)}},[i,f,p,m,N,d]),[ma,ua]=t.useState(()=>tt().id),pa=t.useCallback(a=>{st(a),ua(a)},[]),[Q,ye]=t.useState(!1),[M,ke]=t.useState({done:0,total:0,label:""}),[Ue,_]=t.useState(null),xa=t.useCallback(async()=>{if(g){ye(!0),_(null),ke({done:0,total:0,label:""});try{const a=await g((s,l,n)=>ke({done:s,total:l,label:n}));a.error?_(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?_("Nu mai exista documente de mutat — totul e deja in Storage."):_(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){_(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{ye(!1)}}},[g]),pe=Ca(r,"manageUsers"),[Me,ba]=t.useState([]),[Fe,q]=t.useState(""),F=t.useCallback(async()=>{const a=await Xa();ba(a)},[]);t.useEffect(()=>{pe&&F()},[pe,F]);const Ta=t.useCallback(async(a,s)=>{q("");const{error:l}=await Je(a.id,{role:s,approved:!0});l?q(l):F()},[F]),ha=t.useCallback(async a=>{q("");const{error:s}=await Je(a.id,{approved:!a.approved});s?q(s):F()},[F]),Ea=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),Be=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
