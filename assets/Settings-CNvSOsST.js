...
`,xt="2026-09-22 10:02",Tt=i=>{if(i===1)return"zi";const x=i%100;return x===0||x>=20?"de zile":"zile"},vt=({devices:i,invoices:x=[],tasks:E=[],referate:u=[],foundationDocs:g=[],comenzi:d=[],onImport:N,auditLog:c=[],currentUser:o=null,onMigrateFiles:S,deletions:R=[],onRestore:v,canDelete:m=!1,onUnesteSectii:j,canEdit:A=!1})=>{t.useRef(null);const[J,W]=t.useState(Xa()),[V,n]=t.useState(J.url||""),[T,h]=t.useState(J.key||""),[C,f]=t.useState(!1),[L,H]=t.useState(!1),[bt,ht]=t.useState(null),[X,le]=t.useState(null),[je,Ae]=t.useState(!1),[na,ca]=t.useState(null),[ie,oa]=t.useState(null),[O,da]=t.useState(null),[Se,ve]=t.useState(!1),[y,Le]=t.useState(()=>qa());t.useEffect(()=>{et().then(Le)},[]);const G=t.useMemo(()=>at(i,x,u,g,d),[i,x,u,g,d]),re=t.useCallback(async()=>{ve(!0);const[a,s]=await Promise.all([tt(),st()]);oa(a),da(s),ve(!1)},[]);t.useEffect(()=>{re()},[re]);const[ma,we]=t.useState(null),[ne,Ie]=t.useState(!1),[w,Ce]=t.useState(null),[ce,oe]=t.useState(null),[de,Xe]=t.useState(!1),[K,Z]=t.useState(!1),[Oe,De]=t.useState(0),[$,D]=t.useState(null),F=t.useCallback(async()=>{if(!ae){oe("Cloud neconfigurat");return}Xe(!0),oe(null);const{count:a,error:s}=await Oa("devices");s?(oe(s.message||"eroare necunoscuta"),Ce(null)):Ce(a),Xe(!1)},[]);t.useEffect(()=>{F()},[F,i.length]);const[I,Re]=t.useState(null),[me,ue]=t.useState(!1),ua=t.useCallback(async()=>{ue(!0),Re(null);const{data:a,error:s}=await Je("devices");if(s||!a){D({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),ue(!1);return}const l=new Set(a.map(p=>String(p.id).trim())),r=new Set(i.map(p=>String(p.id).trim())),b=p=>{var U,ee;return((U=i.find(Te=>Te.id===p))==null?void 0:U.name)||((ee=a.find(Te=>String(Te.id).trim()===p))==null?void 0:ee.name)||p};Re({localOnly:[...r].filter(p=>!l.has(p)).map(p=>`${b(p)} (${p})`),cloudOnly:[...l].filter(p=>!r.has(p)).map(p=>`${b(p)} (${p})`)}),ue(!1)},[i]),pa=t.useCallback(async()=>{if(i.length===0)return;Z(!0),De(0),D(null);const{data:a,error:s}=await Je("devices");if(s){Z(!1),D({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=Da(i,a||[]);if(l.length===0){Z(!1),D({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await F();return}const{error:r,written:b,skippedColumns:p,oversized:U}=await Ra("devices",l,100,ee=>De(ee));Z(!1),r?D({ok:!1,message:`Urcarea s-a oprit dupa ${b} echipamente: ${r.message||r}`}):U.length>0?D({ok:!1,message:`${b} echipamente urcate, dar ${U.length} nu au incaput (documente atasate prea mari): ${U.slice(0,3).join(", ")}${U.length>3?"...":""}`}):p.length>0?D({ok:!0,message:`${b} echipamente au fost urcate. Atentie: campurile ${p.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):D({ok:!0,message:`${b} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await F()},[i,F]),k=t.useMemo(()=>{let a=0,s=0;const l=r=>{r!=null&&r.startsWith("data:")&&(a++,s+=Math.round(r.length*.75))};return i.forEach(r=>{(r.files||[]).forEach(b=>{b.path||l(b.url)}),(r.contracts||[]).forEach(b=>{b.filePath||l(b.fileUrl)})}),E.forEach(r=>(r.attachments||[]).forEach(b=>{b.path||l(b.url)})),[...x,...u,...g,...d].forEach(r=>{r.filePath||l(r.fileUrl)}),{count:a,mb:s/(1024*1024)}},[i,E,x,u,g,d]),[xa,Ta]=t.useState(()=>nt().id),[ba,ha]=t.useState(()=>ct()),Ea=t.useCallback(a=>{ot(a),Ta(a)},[]),[Q,ye]=t.useState(!1),[B,ke]=t.useState({done:0,total:0,label:""}),[Ue,Y]=t.useState(null),Na=t.useCallback(async()=>{if(S){ye(!0),Y(null),ke({done:0,total:0,label:""});try{const a=await S((s,l,r)=>ke({done:s,total:l,label:r}));a.error?Y(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?Y("Nu mai exista documente de mutat — totul e deja in Storage."):Y(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){Y(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{ye(!1)}}},[S]),pe=ya(o,"manageUsers"),[Me,fa]=t.useState([]),[Fe,q]=t.useState(""),z=t.useCallback(async()=>{const a=await ka();fa(a)},[]);t.useEffect(()=>{pe&&z()},[pe,z]);const ga=t.useCallback(async(a,s)=>{q("");const{error:l}=await We(a.id,{role:s,approved:!0});l?q(l):z()},[z]),ja=t.useCallback(async a=>{q("");const{error:s}=await We(a.id,{approved:!a.approved});s?q(s):z()},[z]),Aa=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),Be=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
