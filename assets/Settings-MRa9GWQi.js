import{q as ha,s as M,t as Na,v as ye,w as fa,x as ga,y as ja,z as Aa,A as ke,p as q,S as Ue,B as Sa,j as e,C as La,E as va,Z as ee,N as Ia,R as Me,F as wa,G as Ca}from"./index-BOIxdUdL.js";import{a as t,o as Fe,m as N,v as Be,G as ae,z as Pe,b as Xa,T as Oa,w as Da,C as F,e as te,$ as Ra,aj as _e,d as v,I as ze,a2 as ya,a0 as ka,a7 as Ua,ak as $e,H as Ma,al as Fa,B as Ye,p as Ba}from"./vendor-icons-D1Mfxx8N.js";import{C as Pa}from"./ConfirmDialog-De9d56x-.js";import{i as _a,l as za,s as $a,a as Ya,b as Ja,m as A,N as Wa,L as Ha,p as Ga}from"./spatiu-Bw7AI2vv.js";import{g as Va,s as Ka,S as Je}from"./scanQuality-BG2ILRky.js";import"./vendor-recharts-BEjdFrmV.js";import"./vendor-db-CxvqBt6T.js";const We=`-- BIOMEDIC — REFERATE, DOCUMENTE DE FUNDAMENTARE SI COMENZI
...
`,Za="2026-08-19 17:48",Qa=r=>{if(r===1)return"zi";const T=r%100;return T===0||T>=20?"de zile":"zile"},ot=({devices:r,invoices:T=[],tasks:se=[],referate:I=[],foundationDocs:w=[],comenzi:C=[],onImport:le,auditLog:B=[],currentUser:o=null,onMigrateFiles:X,deletions:He=[],onRestore:ie,canDelete:re=!1})=>{t.useRef(null);const[ne,qa]=t.useState(ha()),[ce,Ge]=t.useState(ne.url||""),[oe,Ve]=t.useState(ne.key||""),[P,Ke]=t.useState(!1),[de,ue]=t.useState(!1),[et,at]=t.useState(null),[m,_]=t.useState(null),[me,pe]=t.useState(!1),[Ze,Qe]=t.useState(null),[z,qe]=t.useState(null),[p,ea]=t.useState(null),[xe,Te]=t.useState(!1),[E,Ee]=t.useState(()=>_a());t.useEffect(()=>{za().then(Ee)},[]);const O=t.useMemo(()=>$a(r,T,I,w,C),[r,T,I,w,C]),$=t.useCallback(async()=>{Te(!0);const[a,s]=await Promise.all([Ya(),Ja()]);qe(a),ea(s),Te(!1)},[]);t.useEffect(()=>{$()},[$]);const[aa,be]=t.useState(null),[Y,he]=t.useState(!1),[d,Ne]=t.useState(null),[J,W]=t.useState(null),[H,fe]=t.useState(!1),[D,R]=t.useState(!1),[ge,je]=t.useState(0),[S,x]=t.useState(null),f=t.useCallback(async()=>{if(!M){W("Cloud neconfigurat");return}fe(!0),W(null);const{count:a,error:s}=await Na("devices");s?(W(s.message||"eroare necunoscuta"),Ne(null)):Ne(a),fe(!1)},[]);t.useEffect(()=>{f()},[f,r.length]);const[u,Ae]=t.useState(null),[G,V]=t.useState(!1),ta=t.useCallback(async()=>{V(!0),Ae(null);const{data:a,error:s}=await ye("devices");if(s||!a){x({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),V(!1);return}const l=new Set(a.map(n=>String(n.id).trim())),i=new Set(r.map(n=>String(n.id).trim())),c=n=>{var h,U;return((h=r.find(Q=>Q.id===n))==null?void 0:h.name)||((U=a.find(Q=>String(Q.id).trim()===n))==null?void 0:U.name)||n};Ae({localOnly:[...i].filter(n=>!l.has(n)).map(n=>`${c(n)} (${n})`),cloudOnly:[...l].filter(n=>!i.has(n)).map(n=>`${c(n)} (${n})`)}),V(!1)},[r]),sa=t.useCallback(async()=>{if(r.length===0)return;R(!0),je(0),x(null);const{data:a,error:s}=await ye("devices");if(s){R(!1),x({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=fa(r,a||[]);if(l.length===0){R(!1),x({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await f();return}const{error:i,written:c,skippedColumns:n,oversized:h}=await ga("devices",l,100,U=>je(U));R(!1),i?x({ok:!1,message:`Urcarea s-a oprit dupa ${c} echipamente: ${i.message||i}`}):h.length>0?x({ok:!1,message:`${c} echipamente urcate, dar ${h.length} nu au incaput (documente atasate prea mari): ${h.slice(0,3).join(", ")}${h.length>3?"...":""}`}):n.length>0?x({ok:!0,message:`${c} echipamente au fost urcate. Atentie: campurile ${n.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):x({ok:!0,message:`${c} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await f()},[r,f]),b=t.useMemo(()=>{let a=0,s=0;const l=i=>{i!=null&&i.startsWith("data:")&&(a++,s+=Math.round(i.length*.75))};return r.forEach(i=>{(i.files||[]).forEach(c=>{c.path||l(c.url)}),(i.contracts||[]).forEach(c=>{c.filePath||l(c.fileUrl)})}),se.forEach(i=>(i.attachments||[]).forEach(c=>{c.path||l(c.url)})),[...T,...I,...w,...C].forEach(i=>{i.filePath||l(i.fileUrl)}),{count:a,mb:s/(1024*1024)}},[r,se,T,I,w,C]),[la,ia]=t.useState(()=>Va().id),ra=t.useCallback(a=>{Ka(a),ia(a)},[]),[y,Se]=t.useState(!1),[g,Le]=t.useState({done:0,total:0,label:""}),[ve,L]=t.useState(null),na=t.useCallback(async()=>{if(X){Se(!0),L(null),Le({done:0,total:0,label:""});try{const a=await X((s,l,i)=>Le({done:s,total:l,label:i}));a.error?L(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?L("Nu mai exista documente de mutat — totul e deja in Storage."):L(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){L(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{Se(!1)}}},[X]),K=ja(o,"manageUsers"),[Ie,ca]=t.useState([]),[we,k]=t.useState(""),j=t.useCallback(async()=>{const a=await Aa();ca(a)},[]);t.useEffect(()=>{K&&j()},[K,j]);const oa=t.useCallback(async(a,s)=>{k("");const{error:l}=await ke(a.id,{role:s,approved:!0});l?k(l):j()},[j]),da=t.useCallback(async a=>{k("");const{error:s}=await ke(a.id,{approved:!a.approved});s?k(s):j()},[j]),ua=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),Ce=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
