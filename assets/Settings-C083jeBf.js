import{m as Qe,o as C,p as Ge,q as Ee,s as Ze,t as qe,v as ea,w as aa,x as he,n as $,S as Ne,y as ta,j as e,z as sa,R as ge,A as la,B as ia}from"./index-BvSABQoa.js";import{a as t,o as fe,m as f,v as je,F as Y,z as Ae,b as na,T as ra,w as ca,C as L,e as H,_ as oa,ai as da,d as X,$ as ua,a7 as ma,aj as Se,G as pa,ak as xa,B as ve,p as Ta}from"./vendor-icons-D4fOy8Ml.js";import{C as ba}from"./ConfirmDialog-ClF7VGkt.js";import{g as Ea,s as ha,S as we}from"./scanQuality-BG2ILRky.js";import"./vendor-recharts-jXshe1Ew.js";import"./vendor-db-CxvqBt6T.js";const Ie=`-- BIOMEDIC — REFERATE SI DOCUMENTE DE FUNDAMENTARE
...
`,Na="2026-08-04 20:40",La=({devices:n,onImport:W,auditLog:O=[],currentUser:r=null,onMigrateFiles:j})=>{t.useRef(null);const[J,ga]=t.useState(Qe()),[V,Ce]=t.useState(J.url||""),[K,Le]=t.useState(J.key||""),[R,Xe]=t.useState(!1),[Q,G]=t.useState(!1),[fa,ja]=t.useState(null),[m,D]=t.useState(null),[Z,q]=t.useState(!1),[Oe,Re]=t.useState(null),[De,ee]=t.useState(null),[y,ae]=t.useState(!1),[d,te]=t.useState(null),[k,U]=t.useState(null),[F,se]=t.useState(!1),[A,S]=t.useState(!1),[le,ie]=t.useState(0),[N,p]=t.useState(null),b=t.useCallback(async()=>{if(!C){U("Cloud neconfigurat");return}se(!0),U(null);const{count:a,error:s}=await Ge("devices");s?(U(s.message||"eroare necunoscuta"),te(null)):te(a),se(!1)},[]);t.useEffect(()=>{b()},[b,n.length]);const[u,ne]=t.useState(null),[M,B]=t.useState(!1),ye=t.useCallback(async()=>{B(!0),ne(null);const{data:a,error:s}=await Ee("devices");if(s||!a){p({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),B(!1);return}const l=new Set(a.map(i=>String(i.id).trim())),c=new Set(n.map(i=>String(i.id).trim())),o=i=>{var T,I;return((T=n.find(z=>z.id===i))==null?void 0:T.name)||((I=a.find(z=>String(z.id).trim()===i))==null?void 0:I.name)||i};ne({localOnly:[...c].filter(i=>!l.has(i)).map(i=>`${o(i)} (${i})`),cloudOnly:[...l].filter(i=>!c.has(i)).map(i=>`${o(i)} (${i})`)}),B(!1)},[n]),ke=t.useCallback(async()=>{if(n.length===0)return;S(!0),ie(0),p(null);const{data:a,error:s}=await Ee("devices");if(s){S(!1),p({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=Ze(n,a||[]);if(l.length===0){S(!1),p({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await b();return}const{error:c,written:o,skippedColumns:i,oversized:T}=await qe("devices",l,100,I=>ie(I));S(!1),c?p({ok:!1,message:`Urcarea s-a oprit dupa ${o} echipamente: ${c.message||c}`}):T.length>0?p({ok:!1,message:`${o} echipamente urcate, dar ${T.length} nu au incaput (documente atasate prea mari): ${T.slice(0,3).join(", ")}${T.length>3?"...":""}`}):i.length>0?p({ok:!0,message:`${o} echipamente au fost urcate. Atentie: campurile ${i.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):p({ok:!0,message:`${o} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await b()},[n,b]),x=t.useMemo(()=>{let a=0,s=0;return n.forEach(l=>(l.files||[]).forEach(c=>{var o;!c.path&&((o=c.url)!=null&&o.startsWith("data:"))&&(a++,s+=Math.round(c.url.length*.75))})),{count:a,mb:s/(1024*1024)}},[n]),[Ue,Fe]=t.useState(()=>Ea().id),Me=t.useCallback(a=>{ha(a),Fe(a)},[]),[v,re]=t.useState(!1),[E,ce]=t.useState({done:0,total:0,label:""}),[oe,g]=t.useState(null),Be=t.useCallback(async()=>{if(j){re(!0),g(null),ce({done:0,total:0,label:""});try{const a=await j((s,l,c)=>ce({done:s,total:l,label:c}));a.error?g(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?g("Nu mai exista documente de mutat — totul e deja in Storage."):g(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){g(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{re(!1)}}},[j]),_=ea(r,"manageUsers"),[de,_e]=t.useState([]),[ue,w]=t.useState(""),h=t.useCallback(async()=>{const a=await aa();_e(a)},[]);t.useEffect(()=>{_&&h()},[_,h]);const Pe=t.useCallback(async(a,s)=>{w("");const{error:l}=await he(a.id,{role:s,approved:!0});l?w(l):h()},[h]),ze=t.useCallback(async a=>{w("");const{error:s}=await he(a.id,{approved:!a.approved});s?w(s):h()},[h]),$e=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),me=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
