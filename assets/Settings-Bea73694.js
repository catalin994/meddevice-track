import{m as Qe,o as w,p as Ge,q as be,s as Ze,t as qe,v as ea,w as aa,x as he,n as $,S as Ne,y as ta,j as e,z as sa,R as ge,A as la,B as ia}from"./index-BlbwuyLW.js";import{a as t,o as fe,m as f,v as Ae,G as Y,z as je,b as ra,T as na,w as ca,C,e as H,$ as oa,aj as da,d as X,a0 as ua,a8 as ma,ak as Se,H as pa,al as xa,B as Le,p as Ta}from"./vendor-icons-BHIXBo1P.js";import{C as Ea}from"./ConfirmDialog-B1ywaEvg.js";import{g as ba,s as ha,S as ve}from"./scanQuality-BG2ILRky.js";import"./vendor-recharts-DG5PL2wZ.js";import"./vendor-db-CxvqBt6T.js";const Ie=`-- BIOMEDIC — REFERATE SI DOCUMENTE DE FUNDAMENTARE
...
`,Na="2026-08-05 12:15",Ca=({devices:r,onImport:W,auditLog:O=[],currentUser:n=null,onMigrateFiles:A})=>{t.useRef(null);const[J,ga]=t.useState(Qe()),[V,we]=t.useState(J.url||""),[K,Ce]=t.useState(J.key||""),[D,Xe]=t.useState(!1),[Q,G]=t.useState(!1),[fa,Aa]=t.useState(null),[m,R]=t.useState(null),[Z,q]=t.useState(!1),[Oe,De]=t.useState(null),[Re,ee]=t.useState(null),[y,ae]=t.useState(!1),[d,te]=t.useState(null),[k,U]=t.useState(null),[F,se]=t.useState(!1),[j,S]=t.useState(!1),[le,ie]=t.useState(0),[N,p]=t.useState(null),E=t.useCallback(async()=>{if(!w){U("Cloud neconfigurat");return}se(!0),U(null);const{count:a,error:s}=await Ge("devices");s?(U(s.message||"eroare necunoscuta"),te(null)):te(a),se(!1)},[]);t.useEffect(()=>{E()},[E,r.length]);const[u,re]=t.useState(null),[M,B]=t.useState(!1),ye=t.useCallback(async()=>{B(!0),re(null);const{data:a,error:s}=await be("devices");if(s||!a){p({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),B(!1);return}const l=new Set(a.map(i=>String(i.id).trim())),c=new Set(r.map(i=>String(i.id).trim())),o=i=>{var T,I;return((T=r.find(z=>z.id===i))==null?void 0:T.name)||((I=a.find(z=>String(z.id).trim()===i))==null?void 0:I.name)||i};re({localOnly:[...c].filter(i=>!l.has(i)).map(i=>`${o(i)} (${i})`),cloudOnly:[...l].filter(i=>!c.has(i)).map(i=>`${o(i)} (${i})`)}),B(!1)},[r]),ke=t.useCallback(async()=>{if(r.length===0)return;S(!0),ie(0),p(null);const{data:a,error:s}=await be("devices");if(s){S(!1),p({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=Ze(r,a||[]);if(l.length===0){S(!1),p({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await E();return}const{error:c,written:o,skippedColumns:i,oversized:T}=await qe("devices",l,100,I=>ie(I));S(!1),c?p({ok:!1,message:`Urcarea s-a oprit dupa ${o} echipamente: ${c.message||c}`}):T.length>0?p({ok:!1,message:`${o} echipamente urcate, dar ${T.length} nu au incaput (documente atasate prea mari): ${T.slice(0,3).join(", ")}${T.length>3?"...":""}`}):i.length>0?p({ok:!0,message:`${o} echipamente au fost urcate. Atentie: campurile ${i.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):p({ok:!0,message:`${o} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await E()},[r,E]),x=t.useMemo(()=>{let a=0,s=0;return r.forEach(l=>(l.files||[]).forEach(c=>{var o;!c.path&&((o=c.url)!=null&&o.startsWith("data:"))&&(a++,s+=Math.round(c.url.length*.75))})),{count:a,mb:s/(1024*1024)}},[r]),[Ue,Fe]=t.useState(()=>ba().id),Me=t.useCallback(a=>{ha(a),Fe(a)},[]),[L,ne]=t.useState(!1),[b,ce]=t.useState({done:0,total:0,label:""}),[oe,g]=t.useState(null),Be=t.useCallback(async()=>{if(A){ne(!0),g(null),ce({done:0,total:0,label:""});try{const a=await A((s,l,c)=>ce({done:s,total:l,label:c}));a.error?g(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?g("Nu mai exista documente de mutat — totul e deja in Storage."):g(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){g(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{ne(!1)}}},[A]),_=ea(n,"manageUsers"),[de,_e]=t.useState([]),[ue,v]=t.useState(""),h=t.useCallback(async()=>{const a=await aa();_e(a)},[]);t.useEffect(()=>{_&&h()},[_,h]);const Pe=t.useCallback(async(a,s)=>{v("");const{error:l}=await he(a.id,{role:s,approved:!0});l?v(l):h()},[h]),ze=t.useCallback(async a=>{v("");const{error:s}=await he(a.id,{approved:!a.approved});s?v(s):h()},[h]),$e=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),me=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
