import{j as e,t as va,v as ae,w as wa,x as Ye,y as La,z as Ia,A as Ca,B as Xa,C as Je,s as be,S as We,E as Oa,F as Da,G as Ra,Z as he,N as ya,I as He,J as ka,K as Ua}from"./index-BNHfN8jm.js";import{a as t,ak as Ma,F as se,I as fe,aj as Ve,al as Ee,d as _,o as Ge,m as M,v as Ke,z as Ze,b as Fa,T as Ba,w as za,C as te,e as Ne,a0 as Pa,am as Qe,a3 as _a,a1 as $a,a7 as Ya,an as qe,J as Ja,ao as Wa,B as ea,p as Ha}from"./vendor-icons-GVqSwoQY.js";import{C as sa}from"./ConfirmDialog-KIqxUM8_.js";import{i as Va,l as Ga,s as Ka,a as Za,b as Qa,m as P,N as qa,L as et,p as at}from"./spatiu-DVGIRYyd.js";import{g as tt,s as st,S as aa}from"./scanQuality-BG2ILRky.js";import"./vendor-recharts-BxiADjAJ.js";import"./vendor-db-CxvqBt6T.js";const lt=new Set(["sectia","sectie","sec","compartimentul","compartiment","comp","serviciul","serviciu","serv","unitatea","unitate","clinica","clinic","laborator","laboratorul","laboratoare","lab","de","si","a","al","ale","cu","din","pentru","la"]),it={cab:"cabinet",cabinetul:"cabinet",cabinete:"cabinet",ati:"anestezie terapie intensiva",ti:"terapie intensiva",upu:"unitate primiri urgente",cpu:"camera primiri urgente",uts:"unitate transfuzie sanguina",ctt:"centru transfuzie",bo:"bloc operator",blocop:"bloc operator",orl:"otorinolaringologie",bfk:"balneofizioterapie",rmf:"recuperare medicina fizica"},rt=i=>String(i??"").normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[șşŞȘ]/g,"s").replace(/[țţŢȚ]/g,"t").replace(/[ăâĂÂ]/g,"a").replace(/[îÎ]/g,"i").toLowerCase(),ge=i=>{const x=rt(i).replace(/[^a-z0-9]+/g," ").trim().replace(/\b(?:[a-z] ){1,}[a-z]\b/g,u=>u.replace(/ /g,""));if(!x)return"";const g=x.split(/\s+/).map(u=>it[u]||u).join(" ").split(/\s+/).filter(u=>u&&!lt.has(u));return g.length===0?x:[...new Set(g)].sort().join(" ")},nt=(i,x=[])=>{const g=new Map,u=(d,E)=>{const c=String(d??"").trim();if(!c)return;const o=g.get(c)||{aparate:0,tichete:0};o[E]++,g.set(c,o)};i.forEach(d=>u(d.department,"aparate")),x.forEach(d=>u(d.department,"tichete"));const f=new Map;for(const[d,E]of g){const c=ge(d);if(!c)continue;const o=f.get(c)||{cheie:c,feluri:[],propus:d,total:0};o.feluri.push({nume:d,aparate:E.aparate,tichete:E.tichete}),o.total+=E.aparate+E.tichete,f.set(c,o)}return[...f.values()].filter(d=>d.feluri.length>1).map(d=>{var A;d.feluri.sort((m,I)=>I.aparate+I.tichete-(m.aparate+m.tichete)||m.nume.localeCompare(I.nume,"ro"));const E=m=>/\b[a-zA-ZăâîșțĂÂÎȘȚ]{1,4}\./.test(m),c=d.feluri.filter(m=>!E(m.nume)),o=c.length?c:d.feluri,j=o[0].aparate+o[0].tichete,R=o.filter(m=>m.aparate+m.tichete===j);return d.propus=((A=R.find(m=>/[ăâîșțĂÂÎȘȚşţŞŢ]/.test(m.nume)))==null?void 0:A.nume)||R[0].nume,d}).sort((d,E)=>E.total-d.total)},ct=({devices:i,tasks:x,onUneste:g,canEdit:u})=>{const f=t.useMemo(()=>nt(i,x),[i,x]),[d,E]=t.useState({}),[c,o]=t.useState(null),[j,R]=t.useState(!1),A=t.useMemo(()=>{const n=new Set;return i.forEach(T=>{var h;(h=T.department)!=null&&h.trim()&&n.add(T.department.trim())}),x.forEach(T=>{var h;(h=T.department)!=null&&h.trim()&&n.add(T.department.trim())}),[...n].sort((T,h)=>T.localeCompare(h,"ro"))},[i,x]),[m,I]=t.useState(""),[S,J]=t.useState(""),W=n=>i.filter(T=>{var h;return((h=T.department)==null?void 0:h.trim())===n}).length+x.filter(T=>{var h;return((h=T.department)==null?void 0:h.trim())===n}).length,H=async()=>{if(c){R(!0);try{await g(c.dela,c.la)}finally{R(!1),o(null)}}};return e.jsxs("div",{className:"bg-white p-6 sm:p-10 rounded-[2rem] shadow-sm border border-slate-100",children:[e.jsxs("div",{className:"flex items-center gap-3 sm:gap-5 mb-6",children:[e.jsx("div",{className:"p-3 sm:p-5 bg-indigo-100 text-indigo-600 rounded-2xl sm:rounded-3xl shrink-0",children:e.jsx(Ma,{className:"w-7 h-7 sm:w-10 sm:h-10"})}),e.jsxs("div",{className:"min-w-0",children:[e.jsx("h2",{className:"text-xl sm:text-3xl font-black text-slate-900 tracking-tight leading-none",children:"Sectii care se repeta"}),e.jsx("p",{className:"text-[12px] sm:text-sm text-slate-500 font-semibold mt-1",children:"Aceeasi sectie scrisa in mai multe feluri, adusa la una singura"})]})]}),f.length===0?e.jsxs("div",{className:"p-5 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-start gap-3",children:[e.jsx(se,{className:"w-5 h-5 text-emerald-600 shrink-0 mt-0.5"}),e.jsxs("p",{className:"text-sm font-semibold text-emerald-800 leading-relaxed",children:["Nicio sectie nu se repeta. ",A.length," sectii, fiecare cu un singur nume."]})]}):e.jsxs("div",{className:"space-y-4",children:[e.jsxs("div",{className:"p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3",children:[e.jsx(fe,{className:"w-4 h-4 text-amber-600 shrink-0 mt-0.5"}),e.jsxs("p",{className:"text-[13px] font-semibold text-amber-900 leading-relaxed",children:[e.jsx("span",{className:"font-black",children:f.length})," ",f.length===1?"sectie apare":"sectii apar",' sub mai multe nume. Alege forma care ramane si apasa "Uneste" — aparatele si tichetele se muta pe ea. Verifica-le pe fiecare: "Chirurgie 1" si "Chirurgie 2" seamana, dar sunt doua sectii.']})]}),f.map(n=>{const T=d[n.cheie]||n.propus,h=n.feluri.filter(N=>N.nume!==T),C=h.reduce((N,v)=>N+v.aparate+v.tichete,0);return e.jsxs("div",{className:"p-4 sm:p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-3",children:[e.jsx("div",{className:"flex flex-wrap gap-2",children:n.feluri.map(N=>{const v=N.nume===T;return e.jsxs("button",{onClick:()=>E(V=>({...V,[n.cheie]:N.nume})),disabled:!u,title:v?"Numele care ramane":"Apasa ca sa pastrezi acest nume",className:`px-3.5 py-2.5 rounded-xl text-[12px] font-bold transition border-2 text-left disabled:cursor-not-allowed ${v?"bg-emerald-600 border-emerald-600 text-white shadow-lg shadow-emerald-600/20":"bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`,children:[e.jsx("span",{className:"block",children:N.nume}),e.jsxs("span",{className:`block text-[10px] font-black uppercase tracking-wide mt-0.5 ${v?"text-white/70":"text-slate-400"}`,children:[N.aparate," aparate",N.tichete?` · ${N.tichete} tichete`:""]})]},N.nume)})}),e.jsxs("div",{className:"flex flex-wrap items-center justify-between gap-3",children:[e.jsxs("p",{className:"text-[12px] font-bold text-slate-500 flex items-center gap-2 min-w-0",children:[e.jsx(Ve,{className:"w-4 h-4 shrink-0 text-slate-400"}),e.jsxs("span",{className:"truncate",children:[C," ",C===1?"inregistrare se muta":"inregistrari se muta"," pe",e.jsxs("span",{className:"text-slate-900 font-black",children:[" ",T]})]})]}),e.jsxs("button",{onClick:()=>o({cheie:n.cheie,dela:h.map(N=>N.nume),la:T,cate:C}),disabled:!u||C===0,className:"px-5 py-2.5 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-wide hover:bg-black transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 shrink-0",children:[e.jsx(Ee,{className:"w-4 h-4"})," Uneste"]})]})]},n.cheie)})]}),e.jsxs("div",{className:"mt-6 pt-6 border-t border-slate-100",children:[e.jsx("p",{className:"text-[11px] font-black text-slate-500 uppercase tracking-wide mb-3",children:"Uneste doua sectii care nu seamana la nume"}),e.jsx("p",{className:"text-[12px] font-semibold text-slate-500 mb-3 leading-relaxed",children:'Pentru cazurile pe care aplicatia n-are cum sa le ghiceasca — "UTS Judetean" si "Unitate de transfuzie", de pilda, cand numele vine din registrul contabil.'}),e.jsxs("div",{className:"flex flex-col sm:flex-row items-stretch sm:items-end gap-3",children:[e.jsxs("label",{className:"flex-1 min-w-0 space-y-1.5",children:[e.jsx("span",{className:"text-[11px] font-bold text-slate-500 uppercase tracking-wide ml-1",children:"Se muta de la"}),e.jsxs("select",{value:m,onChange:n=>I(n.target.value),disabled:!u,className:"w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-blue-500",children:[e.jsx("option",{value:"",children:"— alege sectia —"}),A.map(n=>e.jsxs("option",{value:n,children:[n," (",W(n),")"]},n))]})]}),e.jsx(Ve,{className:"w-5 h-5 text-slate-300 shrink-0 hidden sm:block mb-3.5"}),e.jsxs("label",{className:"flex-1 min-w-0 space-y-1.5",children:[e.jsx("span",{className:"text-[11px] font-bold text-slate-500 uppercase tracking-wide ml-1",children:"Ramane"}),e.jsxs("select",{value:S,onChange:n=>J(n.target.value),disabled:!u,className:"w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-blue-500",children:[e.jsx("option",{value:"",children:"— alege sectia —"}),A.map(n=>e.jsxs("option",{value:n,children:[n," (",W(n),")"]},n))]})]}),e.jsxs("button",{onClick:()=>o({cheie:"manual",dela:[m],la:S,cate:W(m)}),disabled:!u||!m||!S||m===S,className:"px-6 py-3 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-wide hover:bg-black transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 shrink-0",children:[e.jsx(Ee,{className:"w-4 h-4"})," Uneste"]})]}),m&&S&&m!==S&&ge(m)!==ge(S)&&e.jsx("p",{className:"text-[12px] font-bold text-amber-700 mt-3 leading-relaxed",children:"Numele astea doua nu seamana deloc. Sigur e aceeasi sectie?"})]}),e.jsx(sa,{open:!!c,tone:"neutral",title:"Unesti sectiile?",icon:j?e.jsx(_,{className:"w-8 h-8 animate-spin"}):e.jsx(Ee,{className:"w-8 h-8"}),body:e.jsxs(e.Fragment,{children:[e.jsx("span",{className:"font-black text-slate-900",children:c==null?void 0:c.cate})," ",(c==null?void 0:c.cate)===1?"inregistrare se muta":"inregistrari se muta"," de pe"," ",c==null?void 0:c.dela.map(n=>e.jsxs("span",{className:"font-black text-slate-900",children:['"',n,'" ']},n)),"pe ",e.jsxs("span",{className:"font-black text-slate-900",children:['"',c==null?void 0:c.la,'"']}),". Aparatele raman aceleasi, li se schimba doar sectia — si se poate face la loc de aici, unind inapoi."]}),confirmLabel:j?"Se muta...":"Uneste",cancelLabel:"Renunt",onCancel:()=>o(null),onConfirm:H})]})},ta=`-- BIOMEDIC — REFERATE, DOCUMENTE DE FUNDAMENTARE SI COMENZI
...
`,ot="2026-09-03 21:20",dt=i=>{if(i===1)return"zi";const x=i%100;return x===0||x>=20?"de zile":"zile"},ft=({devices:i,invoices:x=[],tasks:g=[],referate:u=[],foundationDocs:f=[],comenzi:d=[],onImport:E,auditLog:c=[],currentUser:o=null,onMigrateFiles:j,deletions:R=[],onRestore:A,canDelete:m=!1,onUnesteSectii:I,canEdit:S=!1})=>{t.useRef(null);const[J,W]=t.useState(va()),[H,n]=t.useState(J.url||""),[T,h]=t.useState(J.key||""),[C,N]=t.useState(!1),[v,V]=t.useState(!1),[mt,ut]=t.useState(null),[X,le]=t.useState(null),[je,Ae]=t.useState(!1),[la,ia]=t.useState(null),[ie,ra]=t.useState(null),[O,na]=t.useState(null),[Se,ve]=t.useState(!1),[y,we]=t.useState(()=>Va());t.useEffect(()=>{Ga().then(we)},[]);const G=t.useMemo(()=>Ka(i,x,u,f,d),[i,x,u,f,d]),re=t.useCallback(async()=>{ve(!0);const[a,s]=await Promise.all([Za(),Qa()]);ra(a),na(s),ve(!1)},[]);t.useEffect(()=>{re()},[re]);const[ca,Le]=t.useState(null),[ne,Ie]=t.useState(!1),[w,Ce]=t.useState(null),[ce,oe]=t.useState(null),[de,Xe]=t.useState(!1),[K,Z]=t.useState(!1),[Oe,De]=t.useState(0),[$,D]=t.useState(null),F=t.useCallback(async()=>{if(!ae){oe("Cloud neconfigurat");return}Xe(!0),oe(null);const{count:a,error:s}=await wa("devices");s?(oe(s.message||"eroare necunoscuta"),Ce(null)):Ce(a),Xe(!1)},[]);t.useEffect(()=>{F()},[F,i.length]);const[L,Re]=t.useState(null),[me,ue]=t.useState(!1),oa=t.useCallback(async()=>{ue(!0),Re(null);const{data:a,error:s}=await Ye("devices");if(s||!a){D({ok:!1,message:`Nu s-a putut citi lista din cloud: ${(s==null?void 0:s.message)||"eroare"}`}),ue(!1);return}const l=new Set(a.map(p=>String(p.id).trim())),r=new Set(i.map(p=>String(p.id).trim())),b=p=>{var U,ee;return((U=i.find(Te=>Te.id===p))==null?void 0:U.name)||((ee=a.find(Te=>String(Te.id).trim()===p))==null?void 0:ee.name)||p};Re({localOnly:[...r].filter(p=>!l.has(p)).map(p=>`${b(p)} (${p})`),cloudOnly:[...l].filter(p=>!r.has(p)).map(p=>`${b(p)} (${p})`)}),ue(!1)},[i]),da=t.useCallback(async()=>{if(i.length===0)return;Z(!0),De(0),D(null);const{data:a,error:s}=await Ye("devices");if(s){Z(!1),D({ok:!1,message:`Nu s-a putut citi cloud-ul inainte de urcare: ${s.message||s}`});return}const l=La(i,a||[]);if(l.length===0){Z(!1),D({ok:!0,message:"Cloud-ul are deja tot ce exista pe acest dispozitiv — nimic de urcat."}),await F();return}const{error:r,written:b,skippedColumns:p,oversized:U}=await Ia("devices",l,100,ee=>De(ee));Z(!1),r?D({ok:!1,message:`Urcarea s-a oprit dupa ${b} echipamente: ${r.message||r}`}):U.length>0?D({ok:!1,message:`${b} echipamente urcate, dar ${U.length} nu au incaput (documente atasate prea mari): ${U.slice(0,3).join(", ")}${U.length>3?"...":""}`}):p.length>0?D({ok:!0,message:`${b} echipamente au fost urcate. Atentie: campurile ${p.join(", ")} nu exista in tabelul din cloud si au fost omise — ruleaza scriptul SQL de mai sus in Supabase, apoi urca din nou ca sa se salveze si ele.`}):D({ok:!0,message:`${b} echipamente au fost urcate in cloud. Deschide aplicatia pe celalalt telefon si apasa Re-sincronizare.`}),await F()},[i,F]),k=t.useMemo(()=>{let a=0,s=0;const l=r=>{r!=null&&r.startsWith("data:")&&(a++,s+=Math.round(r.length*.75))};return i.forEach(r=>{(r.files||[]).forEach(b=>{b.path||l(b.url)}),(r.contracts||[]).forEach(b=>{b.filePath||l(b.fileUrl)})}),g.forEach(r=>(r.attachments||[]).forEach(b=>{b.path||l(b.url)})),[...x,...u,...f,...d].forEach(r=>{r.filePath||l(r.fileUrl)}),{count:a,mb:s/(1024*1024)}},[i,g,x,u,f,d]),[ma,ua]=t.useState(()=>tt().id),pa=t.useCallback(a=>{st(a),ua(a)},[]),[Q,ye]=t.useState(!1),[B,ke]=t.useState({done:0,total:0,label:""}),[Ue,Y]=t.useState(null),xa=t.useCallback(async()=>{if(j){ye(!0),Y(null),ke({done:0,total:0,label:""});try{const a=await j((s,l,r)=>ke({done:s,total:l,label:r}));a.error?Y(`S-au mutat ${a.moved} din ${a.total}, apoi a aparut o eroare: ${a.error}`):a.total===0?Y("Nu mai exista documente de mutat — totul e deja in Storage."):Y(`Gata: ${a.moved} documente mutate in Storage.`)}catch(a){Y(`Mutarea a esuat: ${(a==null?void 0:a.message)||a}`)}finally{ye(!1)}}},[j]),pe=Ca(o,"manageUsers"),[Me,Ta]=t.useState([]),[Fe,q]=t.useState(""),z=t.useCallback(async()=>{const a=await Xa();Ta(a)},[]);t.useEffect(()=>{pe&&z()},[pe,z]);const ba=t.useCallback(async(a,s)=>{q("");const{error:l}=await Je(a.id,{role:s,approved:!0});l?q(l):z()},[z]),ha=t.useCallback(async a=>{q("");const{error:s}=await Je(a.id,{approved:!a.approved});s?q(s):z()},[z]),Ea=t.useCallback(async()=>{try{if("caches"in window){const s=await caches.keys();await Promise.all(s.map(l=>caches.delete(l)))}if("serviceWorker"in navigator){const s=await navigator.serviceWorker.getRegistrations();await Promise.all(s.map(l=>l.unregister()))}}catch{}const a=new URL(window.location.href);a.searchParams.set("v",Date.now().toString()),window.location.replace(a.toString())},[]),Be=`-- BIOMEDIC — SCHEMA + MIGRARE (se poate rula de mai multe ori, in siguranta)
