...
const SablonPanel: React.FC<{ fel: FelSablon; titlu: string }> = ({ fel, titlu }) => {
          ...
          <div className="fixed inset-0 z-[620] scrim flex items-center justify-center p-0 sm:p-6">
            <div className="bg-white w-full max-w-2xl h-[100dvh] sm:h-auto sm:max-h-[92dvh] overflow-hidden flex flex-col rounded-none sm:rounded-3xl shadow-2xl animate-slide-up">
              <div className="flex items-center justify-between px-6 sm:px-8 py-5 border-b border-slate-100 shrink-0">
                <div>
                  <h3 className="text-lg font-black text-slate-900 tracking-tight">Sablon Word</h3>
                  <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mt-0.5">{titlu}</p>
                </div>
 ...
 are ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <label className={`flex-1 flex items-center justify-center gap-2 px-5 py-4 rounded-2xl text-[13px] font-bold tracking-normal cursor-pointer transition ${
                  ...
                  {sursa === 'propriu' && (
                    <button onClick={sterge} disabled={lucreaza}
                      ...
                      className="px-5 py-4 bg-slate-50 text-slate-600 border-2 border-slate-200 rounded-2xl text-[13px] font-bold tracking-normal hover:text-red-600 hover:border-red-200 transition flex items-center justify-center gap-2">
