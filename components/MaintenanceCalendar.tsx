...
const MaintenanceCalendar: React.FC<MaintenanceCalendarProps> = ({ devices, onReschedule, onSelectDevice }) => {
    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div className="p-6 sm:p-8 flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-black text-slate-900 tracking-tight">{MONTH_NAMES[month]} {year}</h3>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-wide">{monthTotal} mentenante programate</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setYear(today.getFullYear()); setMonth(today.getMonth()); setSelectedDay(null); }}
            className="px-4 py-2.5 bg-slate-50 text-slate-500 hover:text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-wide transition">Azi</button>
      ...
      <div className="p-4 sm:p-6">
        <div className="grid grid-cols-7 mb-2">
          {WEEKDAYS.map(w => <p key={w} className="text-center text-[10px] font-black text-slate-500 uppercase tracking-wide py-2">{w}</p>)}
        </div>
 ...
 ${isSelected ? 'bg-white/15 text-white' : isPast ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-600'}`}
        ...
        <p className="mt-4 text-center text-[10px] font-bold text-slate-500 uppercase tracking-wide">
      ...
      {selectedDay && (
        <div className="border-t border-slate-100 p-6 sm:p-8 bg-slate-50/50 animate-slide-up">
          ...
          {selectedDevices.length === 0 ? (
            <p className="text-[13px] font-bold text-slate-500 tracking-normal py-4 text-center">Nicio mentenanta programata in aceasta zi</p>
          ) : (
